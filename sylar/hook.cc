...
static sylar::ConfigVar<int>::ptr g_top_connect_timeout = 
    sylar::Config::Lookup("top.connect.timeout", 5000, "top connect timeout");
...
struct _HookIniter {
    _HookIniter() {
        ...
        g_top_connect_timeout->addListener([](const int& old_value, const int& new_value) {
            SYLAR_LOG_INFO(g_logger) << "top connect timeout changed from " << old_value 
                << " to" << new_value;
        });
    }
};
